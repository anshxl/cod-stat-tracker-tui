...
Clicking **Link / sync CSV** also **imports** any matches the file has that the browser doesn't — merged on `(Date, Opponent, Map)`, browser winning collisions. So rows you hand-paste into `ops_kills.csv` (e.g. matches collected before this tool existed) get pulled into the browser on reconnect and then survive every later write. Auto-writes after a save/delete don't re-read the file, so deleting a match in the browser still removes it — only the explicit reconnect merges.
