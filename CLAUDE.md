...
- **Two-way on reconnect:** clicking "Link / sync CSV" runs `readAndMerge()` before writing — `matchesFromCsv()` reconstructs matches from the file (5 player rows → 1 match; `parseCsv()` honors quoted cells) and `mergeMatches()` unions them keyed on `(Date, Opponent, Map)`, browser winning collisions. This imports hand-pasted rows so they survive later overwrites. Auto-writes (save/delete) deliberately skip the read so browser deletes aren't resurrected. A CSV-imported match has `footageMin` but no `start`/`end`; `footageMin(m)` falls back to the stored value.
